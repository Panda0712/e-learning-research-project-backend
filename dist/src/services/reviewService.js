...
const mapReview = (review) => {
    const studentName = review.studentName ||
    ...
    return {
        ...
        studentName,
        studentAvatar: review.studentAvatar || review.student?.avatar?.fileUrl || null,
        rating: review.rating || 0,
    };
};
const getHighlightReviews = async (limit = 3) => {
    try {
...
const getReviewsByCourseId = async (courseId, limit = 10) => {
    try {
        const reviews = await prisma.courseReview.findMany({
...
const getReviewsByCourseIdV2 = async (courseId, page = DEFAULT_PAGE, itemsPerPage = DEFAULT_ITEMS_PER_PAGE, limit = 10) => {
    const currentPage = Number(page) > 0 ? Number(page) : DEFAULT_PAGE;
    const perPage = Number(itemsPerPage) > 0 ? Number(itemsPerPage) : DEFAULT_ITEMS_PER_PAGE;
