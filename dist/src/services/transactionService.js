    ...
    try {
        ...
        for (const item of items) {
            if (item.discountCode) {
                ...
                if (!coupon) {
                    throw new ApiError(StatusCodes.NOT_FOUND, `Discount code: '${item.discountCode}' does not exist!`);
                }
                if (coupon.quantity !== null && coupon.quantity <= 0) {
                    throw new ApiError(StatusCodes.CONFLICT, `Code '${item.discountCode}' has been used up!`);
                }
                const now = new Date();
                if (coupon.endingDate && new Date(coupon.endingDate) < now) {
                    throw new ApiError(StatusCodes.CONFLICT, `Code '${item.discountCode}' has expired!`);
                }
                if (coupon.startingDate && new Date(coupon.startingDate) > now) {
                    throw new ApiError(StatusCodes.CONFLICT, `Code '${item.discountCode}' is not available yet!`);
                }
            }
        }
...
const getStudentTransactionsByCourseId = async (courseId) => {
    try {
...
const getAllTransactions = async () => {
    try {
        ...
        const courseIds = transactions.flatMap(t => t.studentTransactions.map(st => st.courseId).filter(id => id !== null));
