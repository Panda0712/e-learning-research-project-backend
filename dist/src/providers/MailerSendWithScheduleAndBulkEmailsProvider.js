...
const sendEmail = async ({ to, toName, subject, html, templateId, personalizationData, attachments, sendAt, }) => {
    try {
        ...
        // ];
        // send bulk emails codebase
        // const bulkEmails = [];
        // const emailParams = new EmailParams()
        //   .setFrom(sentFrom)
        //   .setTo([new Recipient("your@client.com", "Your Client")])
        ...
        //   .setFrom(sentFrom)
        //   .setTo([new Recipient("your_2@client.com", "Your Client 2")])
        ...
        const buildAttachments = attachments.map((att) => {
            return new Attachment(fs.readFileSync(att.filePath, { encoding: "base64" }), att.fileName, att.attachmentType, att.fileId);
        });
        const emailParams = new EmailParams()
            .setFrom(sendFrom)
            .setTo(recipients)
...
export const MailerSendWithScheduleAndBulkEmailsProvider = { sendEmail };
//# sourceMappingURL=MailerSendWithScheduleAndBulkEmailsProvider.js.map