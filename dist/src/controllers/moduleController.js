...
const updateModule = async (req, res, next) => {
    try {
        const { id } = req.params;
        const updatedModule = await moduleService.updateModule(Number(id), req.body);
...
const getAllModulesByCourseId = async (req, res, next) => {
    try {
        ...
        const modules = await moduleService.getAllModulesByCourseId(Number(courseId), Number(actorId));
