...
const createWishlist = async (data) => {
    try {
        ...
        if (existingWishlist) {
            throw new ApiError(StatusCodes.BAD_REQUEST, "Course already in wishlist!");
        }
...
const checkCourseInWishlist = async (userId, courseId) => {
    try {
...
const updateWishlist = async (id, data) => {
    try {
        ...
        if (data.lecturer !== undefined)
            updateData.lecturer = data.lecturer;
...
const deleteWishlistByCourse = async (userId, courseId) => {
    try {
        const wishlist = await prisma.wishlist.findFirst({
            where: {
                userId,
