...
const createCourse = async (req, res, next) => {
    const correctCondition = Joi.object({
        ...
        status: Joi.string()
            .required()
            .valid(COURSE_STATUS.DRAFT, COURSE_STATUS.PENDING, COURSE_STATUS.PUBLISHED, COURSE_STATUS.REJECTED),
    });
    try {
        await correctCondition.validateAsync(req.body, { abortEarly: false });
        next();
...
const updateCourse = async (req, res, next) => {
    const correctCondition = Joi.object({
        ...
        price: Joi.number().required().min(0),
        status: Joi.string().valid(COURSE_STATUS.DRAFT, COURSE_STATUS.PENDING, COURSE_STATUS.PUBLISHED, COURSE_STATUS.REJECTED),
    });
    try {
...
const approveCourse = async (req, res, next) => {
    ...
    try {
        await correctCondition.validateAsync({ id: Number(req.params.id) }, { abortEarly: false });
        next();
    }
    catch (error) {
        next(new ApiError(StatusCodes.UNPROCESSABLE_ENTITY, new Error(error).message));
...
const getAllCoursesByLecturerId = async (req, res, next) => {
    ...
    try {
        await correctCondition.validateAsync({ lecturerId: Number(req.params.lecturerId) }, { abortEarly: false });
        next();
    }
    catch (error) {
        next(new ApiError(StatusCodes.UNPROCESSABLE_ENTITY, new Error(error).message));
...
const getAllCoursesByStudentId = async (req, res, next) => {
    ...
    try {
        ...
        await correctCondition.validateAsync({ studentId: Number(studentId) }, {
            abortEarly: false,
        });
        await correctConditionParams.validateAsync({ page: Number(page), itemsPerPage: Number(itemsPerPage), q }, {
            abortEarly: false,
        });
        next();
    }
...
const getAllCoursesByCategoryId = async (req, res, next) => {
    ...
    try {
        await correctCondition.validateAsync({ categoryId: Number(req.params.categoryId) }, { abortEarly: false });
        next();
    }
    catch (error) {
        next(new ApiError(StatusCodes.UNPROCESSABLE_ENTITY, new Error(error).message));
