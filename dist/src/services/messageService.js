...
const resolveStudentLecturerPair = async (userAId, userBId, tx) => {
    if (userAId === userBId) {
...
const sendDirectMessage = async (senderId, input) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (conversationId) {
            ...
            const senderMember = conversation.members.find((member) => member.userId === senderId);
            if (!senderMember) {
                throw new ApiError(StatusCodes.FORBIDDEN, "You do not have permission to send message in this conversation.");
            }
            const recipientMember = conversation.members.find((member) => member.userId !== senderId);
            recipientId = recipientMember?.userId;
        }
        else {
            if (!recipientId) {
                throw new ApiError(StatusCodes.BAD_REQUEST, "recipientId is required when conversationId is missing.");
            }
            ...
            const { studentId, lecturerId } = await resolveStudentLecturerPair(senderId, recipientId, tx);
        ...
        if (!conversationId) {
            throw new ApiError(StatusCodes.INTERNAL_SERVER_ERROR, "Cannot resolve conversation.");
        }
        ...
        if (!updatedConversation) {
            throw new ApiError(StatusCodes.INTERNAL_SERVER_ERROR, "Cannot update conversation.");
        }
        const unreadCounts = Object.fromEntries(updatedConversation.members.map((member) => [
            member.userId.toString(),
            member.unreadCount,
        ]));
        const conversationPayloadForSender = buildConversationPayload(updatedConversation, senderId);
        const conversationPayloadForRecipient = recipientId
            ? buildConversationPayload(updatedConversation, recipientId)
            : null;
        return {
            createdConversation,
            recipientId,
