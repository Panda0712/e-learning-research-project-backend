...
const getAllCourseReviews = async (req, res, next) => {
    try {
        ...
        if (rating)
            filterParams.rating = Number(rating);
...
const getReviewsByCourseId = async (req, res, next) => {
    try {
        ...
        const filterParams = { courseId: Number(courseId) };
        if (page)
            filterParams.page = Number(page);
        if (limit)
            filterParams.limit = Number(limit);
        if (rating)
            filterParams.rating = Number(rating);
        const reviews = await courseReviewService.getReviewsByCourseId(filterParams);
        res.status(StatusCodes.OK).json(reviews);
    }
    catch (error) {
        next(error);
...
const getReviewsByStudentId = async (req, res, next) => {
    try {
        ...
        if (limit)
            filterParams.limit = Number(limit);
        const reviews = await courseReviewService.getReviewsByStudentId(filterParams);
        res.status(StatusCodes.OK).json(reviews);
    }
    catch (error) {
        next(error);
...
const updateCourseReview = async (req, res, next) => {
    try {
        const { id } = req.params;
        const updatedReview = await courseReviewService.updateCourseReview(Number(id), req.body);
...
//# sourceMappingURL=courseReviewController.js.map