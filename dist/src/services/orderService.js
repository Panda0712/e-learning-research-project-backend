    ...
    try {
        ...
        if (data.items && data.items.length > 0) {
            ...
            for (const item of data.items) {
                ...
                if (!course) {
                    throw new ApiError(StatusCodes.NOT_FOUND, `Course ${item.courseId} not found!`);
                }
                itemsToOrder.push({
...
const updateOrderStatus = async (orderId, newStatus) => {
    try {
        ...
        if (order.status === "cancelled") {
            throw new ApiError(StatusCodes.BAD_REQUEST, "Cannot update a cancelled order!");
        }
        if (order.status === "completed") {
            throw new ApiError(StatusCodes.BAD_REQUEST, "Cannot update a completed order!");
        }
        // Update order status
...
const cancelOrder = async (orderId) => {
    try {
        ...
        if (order.status !== "pending") {
            throw new ApiError(StatusCodes.BAD_REQUEST, `Cannot cancel order with status: ${order.status}. Only pending orders can be cancelled.`);
        }
