...
export const normalizeConversationRole = (role) => role.toLowerCase();
...
export const pickUserWithAvatar = (user) => {
    if (!user)
        return;
...
export const calculateDateRange = (period, customFrom, customTo) => {
    const now = new Date();
...
export const initChartData = (startDate, endDate, groupBy) => {
    const labels = [];
    const emptyData = [];
    if (groupBy === "month") {
        for (let i = 0; i < 12; i++) {
        ...
        while (tempDate <= endDate) {
            labels.push(tempDate.getDate().toString().padStart(2, "0") +
                "/" +
...
export const getDateIndex = (date, startDate, groupBy) => {
    if (groupBy === "month")
...
export const getPermissionsFromRole = async (roleName) => {
    try {
        ...
        const inheritsArray = typeof role.inherits === "string" ? JSON.parse(role.inherits) : [];
        if (inheritsArray.length > 0) {
            for (const inheritRoleName of inheritsArray) {
                const inheritedPermissions = await getPermissionsFromRole(inheritRoleName);
        ...
        throw new ApiError(StatusCodes.INTERNAL_SERVER_ERROR, "Oops! Something went wrong!");
    }
};
