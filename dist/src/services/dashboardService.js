...
import { calculateDateRange, getDateIndex, initChartData, } from "@/utils/helpers.js";
import { StatusCodes } from "http-status-codes";
// ==========================================
// ADMIN FUNCTIONS
// ==========================================
const ensureAccess = async (input, permissionRole) => {
    ...
    const normalizedRole = String(user.role || input.userRole || "").toLowerCase();
    if (!permissionRole.includes(normalizedRole)) {
        throw new ApiError(StatusCodes.FORBIDDEN, `This account is not allowed to access ${permissionRole} dashboard data!`);
    }
    return user;
};
...
const getAdminCharts = async (actor, period = "this_year", from, to) => {
    try {
        await ensureAccess(actor, [ACCOUNT_ROLES.ADMIN]);
        ...
        users.forEach((u) => {
            ...
            if (idx >= 0 && idx < signupData.length)
                signupData[idx]++;
        });
        revenues.forEach((r) => {
...
// LECTURER FUNCTIONS
// ==========================================
const getLecturerStats = async ({ lecturerId, userRole, }) => {
    try {
        ...
        if (lecturer.role !== "lecturer" && lecturer.role !== "admin") {
            throw new ApiError(StatusCodes.FORBIDDEN, "This account is not a lecturer!");
        }
        const sevenDaysAgo = new Date();
        sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
...
const getLecturerCharts = async ({ lecturerId, userRole }, period = "this_year", from, to) => {
    try {
        await ensureAccess({ userId: lecturerId, userRole }, [
            ACCOUNT_ROLES.ADMIN,
            ACCOUNT_ROLES.LECTURER,
        ...
        enrollments.forEach((e) => {
            ...
            if (idx >= 0 && idx < engagementData.length)
                engagementData[idx]++;
        });
        revenues.forEach((r) => {
            const idx = getDateIndex(new Date(r.createdAt), startDate, groupBy);
