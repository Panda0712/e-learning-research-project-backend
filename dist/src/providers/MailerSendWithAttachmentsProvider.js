...
const sendEmail = async ({ to, toName, subject, html, templateId, personalizationData, attachments, }) => {
    try {
        ...
        const buildAttachments = attachments.map((att) => {
            return new Attachment(fs.readFileSync(att.filePath, { encoding: "base64" }), att.fileName, att.attachmentType);
        });
...
export const MailerSendWithAttachmentsProvider = { sendEmail };
//# sourceMappingURL=MailerSendWithAttachmentsProvider.js.map