...
const createPaymentLink = async (data) => {
    try {
        ...
        if (order.paymentMethod?.toUpperCase() !== "PAYOS") {
            throw new ApiError(StatusCodes.BAD_REQUEST, "This order is not set for PayOS payment!");
        }
        // Kiểm tra đơn hàng chưa thanh toán
        if (order.paymentStatus === "paid") {
            throw new ApiError(StatusCodes.CONFLICT, "This order has already been paid!");
        }
        ...
        const paymentLinkData = {
            ...
            buyerName: `${order.student.firstName || ""} ${order.student.lastName || ""}`.trim(),
        ...
        const paymentLinkResponse = await PayosProvider.createPaymentLink(paymentLinkData);
...
const handleWebhook = async (webhookBody) => {
    try {
        ...
        if (!isValidSignature) {
            ...
            throw new ApiError(StatusCodes.UNAUTHORIZED, "Invalid webhook signature - possible forgery attempt!");
        }
        // Bước 2: Kiểm tra trạng thái giao dịch (code == "00" = thành công)
        const webhookData = PayosProvider.extractWebhookData(webhookBody);
        if (!webhookData) {
            throw new ApiError(StatusCodes.BAD_REQUEST, "Invalid webhook data format!");
        }
        const orderCode = webhookData.data.orderCode;
        const transactionStatus = webhookData.data.status;
        const isSuccessful = webhookData.code === "00" && transactionStatus === "PAID";
...
const cancelPayment = async (orderId) => {
    try {
        ...
        if (order.paymentStatus === "paid") {
            throw new ApiError(StatusCodes.CONFLICT, "Cannot cancel a paid order payment!");
        }
        if (!order.paymentLinkId) {
